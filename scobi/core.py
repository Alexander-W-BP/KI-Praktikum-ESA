...
class Environment(Env):
    ...
    def _draw_objects_overlay(self, obs_image, action=None):
        if len(obs_image.shape) == 1:  # Prüft, ob obs_image ein numerischer Vektor ist
            print("Numerical observation detected; skipping object overlay.")
            return obs_image 
    ...
    def _draw_relation_overlay(self, obs_image, feature_vector, freeze_mask, action=None):
        ...
        if img.mode != "RGBA":
            img = img.convert("RGBA")  # Sicherstellen, dass der Modus korrekt ist
        ...
        # Feature Attribution Normalisierung
        if np.ptp(self.feature_attribution):
            feature_attribution = (
                255 * (self.feature_attribution - np.min(self.feature_attribution)) / np.ptp(self.feature_attribution)
            ).astype(int)
            ...
            for feature in features:
                ...
                # Feature-bezogene Logik
                try:
                    if feature_name == "POSITION":
                        radius = 2
                        x, y = fv_entries[:2]  # Erste zwei Werte sind X, Y
                        coords = (x - radius, y - radius, x + radius, y + radius)
                        draw.ellipse(coords, fill=(0, 0, 0, alpha), outline=(0, 0, 0, alpha))
                    
                    elif feature_name == "POSITION_HISTORY":
                        radius = 2
                        x_t, y_t = fv_entries[2:4]  # Letzte zwei Werte sind X, Y der Vergangenheit
                    ...
                    elif feature_name in ["DISTANCE", "EUCLIDEAN_DISTANCE"]:
                        source_object_coords = feature_signature[0]
                        target_object_coords = feature_signature[1]
                        # Hier könnten die Berechnungen angepasst werden, falls LunarLander andere Daten bereitstellt
                        idx_source = np.where(fv_backmap == features.index(source_object_coords))[0]
                        idx_target = np.where(fv_backmap == features.index(target_object_coords))[0]
                        
                        source_values = feature_vector[idx_source[0]:idx_source[-1] + 1]
                        target_values = feature_vector[idx_target[0]:idx_target[-1] + 1]
                        draw.line(source_values[:2] + target_values[:2], fill=(0, 0, 255, alpha), width=1)
                    ...
                    print(f"Skipping feature '{feature_name}' due to missing data or invalid index.")
...
def mark_bb(image_array, xywh, color=(255, 0, 0), name=""):
    """
    Markiere ein Bounding Box auf einem Bildarray
    :param image_array: Das Bild, das bearbeitet werden soll
    :param xywh: Bounding-Box-Koordinaten im Format (x, y, w, h)
    :param color: Farbe des Rahmens
    :param name: Optionaler Name des Objekts
    """
    # Stelle sicher, dass die Koordinaten Integer sind
    x, y, w, h = map(int, xywh)
    bottom = int(y + h)
    right = int(x + w)

    # Markiere die Bounding Box im Bild
    image_array[y:bottom + 1, x] = color  # Vertikale Linie links
    image_array[y:bottom + 1, right] = color  # Vertikale Linie rechts
    image_array[y, x:right + 1] = color  # Horizontale Linie oben
    image_array[bottom, x:right + 1] = color  # Horizontale Linie unten
